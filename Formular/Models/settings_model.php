...
class settings_Model extends Model {
	...
	public function resetUser_funct(){
		try {
			$query = $this->db->query("SELECT * FROM users WHERE username='".$_POST['Login']."' AND EMAIL='".$_POST['email']."'");
			} catch (PDOException $e)
			{
			...
			try {
			$query = $this->db->query("SELECT * FROM users WHERE username='".$new."'");
			} catch (PDOException $e)
			{
				die($e->getMessage());
			}
			...
			try {
			$var2 = $new;
			$this->db->query("UPDATE users SET username = '".$var2."' WHERE username='".$_POST['Login']."'");	
			} catch (PDOException $e)
			{	
				die($e->getMessage()); 
			}
	...
	public function resetEmail_funct(){
		try {
			$query = $this->db->query("SELECT * FROM users WHERE username='".$_POST['Login']."' AND EMAIL='".$_POST['email']."'");
			} catch (PDOException $e)
			{
			...
			try {
			$var2 = $new;
			$this->db->query("UPDATE users SET EMAIL = '".$var2."' WHERE username='".$_POST['Login']."'");	
			} catch (PDOException $e)
			{	
				die($e->getMessage()); 
			}
